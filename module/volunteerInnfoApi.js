const decodingKey =
  "elnY/olwGOrW1IdetzYnTg57rZ9hax/4cv+wXPk7uUjtqdXNiV7GD5AoPmem3oUzCSTvR0klktDmSF6KuIFJEg==";
const encodingKey =
  "elnY%2FolwGOrW1IdetzYnTg57rZ9hax%2F4cv%2BwXPk7uUjtqdXNiV7GD5AoPmem3oUzCSTvR0klktDmSF6KuIFJEg%3D%3D";
...
const lookUpApi = async () => {
  try {
    // 우선 전체 개수를 알기 위한
    const totalCountApiAddress = await axios.get(
      `http://openapi.1365.go.kr/openapi/service/rest/VolunteerPartcptnService/getVltrPeriodSrvcList?serviceKey=${encodingKey}&progrmBgnde=20220101&progrmEndde=20221231&pageNo=1`
    );
    const totalCount =
      Math.floor(
        Number(totalCountApiAddress.data.response.body.totalCount) /
          Number(totalCountApiAddress.data.response.body.numOfRows)
      ) + 1;
    ...
    for (let d = startNumber; d <= endNumber; d++) {
      ...
      const resultApi = await axios.get(
        `http://openapi.1365.go.kr/openapi/service/rest/VolunteerPartcptnService/getVltrPeriodSrvcList?serviceKey=${encodingKey}&progrmBgnde=20220101&progrmEndde=20221231&pageNo=${d}`
      );
      ...
      for (let n = 0; n < listDateData.length; n++) {
        ...
        detailData.teenPossible =
          detailData.yngbgsPosblAt === "N" ? false : true;
        // 성인 가능
        detailData.adultPossible =
          detailData.adultPosblAt === "N" ? false : true;
        // 그룹 가능
        ...
        // 시간 2글자로 저장
        detailData.actBeginTm =
          String(detailData.actBeginTm).length !== 2
            ? "0" + String(detailData.actBeginTm)
            : String(detailData.actBeginTm);
        detailData.actEndTm =
          String(detailData.actEndTm).length !== 2
            ? "0" + String(detailData.actEndTm)
            : String(detailData.actEndTm);
...
const infoLookUp = async (queryPar) => {
  ...
  const sql =
    "select * from volunteerInfo where teenPossible = ? and adultPossible = ? and groupPossible = ? limit 0 , 5;";
...
const infoCount = async (req, res) => {
  const sql =
    "select count (*) as count from volunteerInfo where progrmBeginDate >= ? and progrmEndDate <= ? and postAdres like ?;";
